...
_marker = []
_allowed_sizes_pattern = re.compile(r'^(.*)\s+(\d+)\s*:\s*(\d+)$')
...
def get_top_site_from_url(context, request):
    """Find the top-most site still present in the current URL path.

    BBB helper for Plone 5 support packages used during the Python 3 stage.
    """
    ...
    try:
        ...
        for idx in range(len(url_path)):
            path = '/'.join(url_path[:idx + 1]) or '/'
            site_path = '/'.join(request.physicalPathFromURL(path)) or '/'
            candidate = context.restrictedTraverse(site_path)
            if ISite.providedBy(candidate):
                site = candidate
                break
...
def getAllowedSizes():
    """Return configured image scale sizes.

    BBB helper for Python-3 import gates that use newer plone.app.vocabularies
    against the local Plone 4.3 CMFPlone source.
    """
    ...
    for line in settings.allowed_sizes:
        line = line.strip()
        if line:
            name, width, height = _allowed_sizes_pattern.match(line).groups()
...
def human_readable_size(value):
    ...
    BBB helper for newer Python-3 support packages used with this Plone 4.3
    source tree.
    """
    smaller = SIZE_ORDER[-1]
    size = value.get_size() if base_hasattr(value, 'get_size') else value
    try:
        size = int(size)
    ...
    if isinstance(size, int):
        if size < SIZE_CONST[smaller]:
            return '1 %s' % smaller
        for unit in SIZE_ORDER:
            if size / SIZE_CONST[unit] > 0:
                break
        return '%.1f %s' % (float(size / float(SIZE_CONST[unit])), unit)
    return size
...
def versionTupleFromString(v_str):
    """Returns version tuple from passed in version string
        ...
        True
        """
    regex_str = r"(^\d+)[.]?(\d*)[.]?(\d*)[- ]?(alpha|beta|candidate|final|a|b|rc)?(\d*)"
    ...
    if match is None:
        ...
        if groups[3] is None:
            groups[3] = 'final'
        elif groups[3] in list(rl_abbr.keys()):
...
def getFSVersionTuple():
    ...
    return versionTupleFromString(version('Products.CMFPlone'))
...
def safe_encode(value, encoding='utf-8', errors='strict'):
    if isinstance(value, bytes):
        return value
    return str(value).encode(encoding, errors)


def safe_nativestring(value, encoding='utf-8', errors='strict'):
    if isinstance(value, bytes):
